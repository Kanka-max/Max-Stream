...
#set sidebar header
st.sidebar.header("Max Liquor Store")

data = st.sidebar.file_uploader("Upload Transactions", type=['csv', 'txt', 'xlsx'])
if data is not None:
        df = pd.read_csv(data)
...
menu = ['Business Snapshot','Analysis','About The Liquor Store']
...
#control statement if...else - menuu                              
if selection== 'Business Snapshot':
    # Use the full page instead of a narrow central column
    #st.beta_set_page_config(layout="wide")
    st.markdown('**My Data**')
    st.table(df_temp.head())

    col1, col2 = st.columns(2)


    with col1:
        st.subheader("Monthly Revenue Overview Bar ")
        df_revenue = df.groupby(["InvoiceMonth", "InvoiceYear"])["Revenue"].sum().reset_index()
        plt.figure(figsize=(15, 10))
        sns.barplot(x="InvoiceMonth", y="Revenue", hue="InvoiceYear", color="salmon", saturation=.8, data=df_revenue)
    ...
    with col3:
        ...
        plt.figure(figsize=(15, 10))
        sns.barplot(x="InvoiceMonth", y="CustomerID", hue="InvoiceYear", color="salmon", saturation=.7, data=df_active)
